...
export function Th({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    ...
    <th className={cn('px-4 py-3 text-right text-xs font-semibold text-slate-500 dark:text-slate-300 whitespace-nowrap', className)}>
      {children}
    </th>
  )
}
...
export function Td({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <td className={cn('px-4 py-3 text-slate-700 dark:text-slate-100 text-right', className)}>
      {children}
    </td>
  )
}
...
export function EmptyRow({ colSpan, message }: { colSpan: number; message: string }) {
  return (
    <tr>
      <td colSpan={colSpan} className="px-4 py-14 text-center text-slate-400 dark:text-slate-400 text-sm">
        {message}
      </td>
    </tr>
  )
