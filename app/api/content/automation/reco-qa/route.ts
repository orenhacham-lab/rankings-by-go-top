  ...
  try {
    ...
    return Response.json({
      ...
      run: {
        ...
        stop_reason_detail: run.diagnostics.brief_consumption,
        ...
        competitorLeakage: run.diagnostics.competitorLeakage,
      ...
      // Manual-quality section: every accepted topic with normalized keyword,
      // demand match type, coverage matches, per-link relevance, external-biz.
      topics: run.suggestions.map((s) => ({
        title: s.title,
        primaryKeyword: s.primaryKeyword,
        normalizedPrimaryKeyword: s.normalizedPrimaryKeyword ?? s.primaryKeyword,
        secondaryKeywords: s.secondaryKeywords,
        intent: s.searchIntent,
        reason: s.suggestionReason,
        demand: s.demandEvidence ?? null,
        demandMatchType: s.demandEvidence?.demandMatchType ?? 'none',
        coverageMatches: s.coverageMatches ?? [],
