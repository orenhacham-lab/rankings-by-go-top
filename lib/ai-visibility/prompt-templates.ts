...
// ============================================================================
// DISPLAY QUALITY GATE — applied to questions from ANY source (DB cache,
// ai_prompts/vNext, Gemini, or fallback) right before they are shown.
//
// Why this exists: displayed questions are loaded from the
// `ai_question_suggestion_cache` table and `ai_prompts` via the
// enriched-suggestions API. Many of those rows were generated by the OLD
// template-based engine BEFORE the intent-based rewrite, so updating
// buildFallbackSuggestions alone never changed what users see. This gate
// rejects those legacy/weak phrasings at display time and lets the caller
// top up with the new engine, with no DB migration.
// ============================================================================
 ...
 * never rejects good Gemini/vNext questions — only the known-bad legacy ones.
 */
export function isLegacyWeakQuestion(
  ...
  // Slug / domain / URL leakage (regardless of language).
  if (/https?:\/\//i.test(q) || /\bwww\./i.test(q)) return true
  if (/[a-z0-9]+\.(com|co|io|net|org|il)\b/i.test(q)) return true
  if (/\b[a-z0-9]+-[a-z0-9]+-[a-z0-9-]+\b/i.test(q)) return true // 3+ token slug

  const HE_LEGACY: RegExp[] = [
    /מה\s+(ה)?יתרונות\s+של/,                 // "מה היתרונות של X?"
    /מומלץ\s+בביקורות/,                       // "האם X מומלץ בביקורות?"
    /למה\s+לבחור\s+.+\s+על\s+פני\s+אלטרנטיבות/, // "למה לבחור X על פני אלטרנטיבות אחרות?"
    /מקור\s+אמין\s+בתחום/,                    // "האם X מקור אמין בתחום?"
    /מי\s+נחשב\s+לאחד\s+המומלצים\s+בתחום/,    // "מי נחשב לאחד המומלצים בתחום X?"
    /מהם\s+החסרונות\s+של/,                    // "מהם החסרונות של X?"
    /איפה\s+אפשר\s+ללמוד\s+עוד\s+על/,         // "איפה אפשר ללמוד עוד על X?"
    /האם\s+זה\s+ממלא\s+תפקיד/,                // "...? האם זה ממלא תפקיד?"
    /על\s+פני\s+אלטרנטיבות\s+אחרות/,          // generic alternatives tail
  ]
  const EN_LEGACY: RegExp[] = [
    /what\s+are\s+the\s+(main\s+)?advantages\s+of/i,     // "What are the main advantages of X?"
    /recommended\s+by\s+users/i,                          // "Is X recommended by users?"
    /why\s+choose\s+.+\s+over\s+(other\s+)?alternatives/i,// "Why choose X over other alternatives?"
    /a\s+trustworthy\s+source/i,                          // "Is X a trustworthy source?"
    /one\s+of\s+the\s+top\s+.+\s+providers/i,             // "Who is considered one of the top X providers?"
    /what\s+are\s+the\s+drawbacks\s+of/i,                 // "What are the drawbacks of X?"
    /where\s+can\s+i\s+learn\s+more\s+about/i,            // "Where can I learn more about X?"
    /does\s+it\s+work\??$/i,                              // "...? Does it work?"
  ]

  const patterns = language === 'he' ? HE_LEGACY : EN_LEGACY
  for (const re of patterns) {
    if (re.test(q)) return true
  }
  // Cross-language safety net: a few legacy strings could appear under either
  // language tag depending on project config — also check the other set's
  // most generic markers.
  if (/a\s+trustworthy\s+source/i.test(q)) return true
  if (/מקור\s+אמין\s+בתחום/.test(q)) return true

  return false
}

export type DisplayGateContext = {
  businessName: string | null
...
export type DisplayGateResult = {
  suggestions: PromptSuggestion[]
  rejectedCount: number
  addedFromEngine: number
  usedEngineTopup: boolean
  generationVersion: string
}

/**
 * Apply the display quality gate to an assembled list of suggestions:
 *   1. Drop legacy/weak/slug questions (isLegacyWeakQuestion).
 *   2. If the surviving count is below `minCount`, top up with fresh
 *      intent-v2 questions from buildFallbackSuggestions (deduped, weak-filtered).
 *
 * Never injects the insufficient-context marker into a non-empty list. If the
 * list becomes empty and the engine itself reports insufficient context, the
 * marker is returned so the UI can show the context state.
 */
export function applyDisplayQualityGate(
  ...
  let usedEngineTopup = false

  if (kept.length < minCount) {
    const engine = buildFallbackSuggestions(
      ctx.businessName ?? null,
    ...
    // The insufficient-context marker is NOT a real question and must never be
    // rendered as one. The top-up loop below skips it; if the engine produced
    // only the marker, `kept` simply stays as-is (possibly empty) and each
    // component's existing empty-state handling takes over.
    const seen = new Set(
      kept.map((s) => s.prompt.trim().toLowerCase().replace(/\s+/g, ' ').replace(/[?.!]+$/, ''))
    )
    for (const e of engine) {
      ...
      kept.push(e)
      addedFromEngine++
      usedEngineTopup = true
    }
  }

  return {
    suggestions: kept.slice(0, maxCount),
    rejectedCount,
    addedFromEngine,
    usedEngineTopup,
