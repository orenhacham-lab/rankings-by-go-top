...
export interface ExpiringOfflineToken {
  ...
  expiresIn: number
  /** Seconds until the REFRESH token expires, as Shopify reported it. */
...
export function parseExpiringOfflineToken(json: unknown): { ok: true; token: ExpiringOfflineToken } | { ok: false; missing: string[] } {
  ...
  if (refreshTokenExpiresIn === null) missing.push('refresh_token_expires_in')
  ...
  return {
    ...
    token: {
      ...
      refreshTokenExpiresIn: refreshTokenExpiresIn as number,
