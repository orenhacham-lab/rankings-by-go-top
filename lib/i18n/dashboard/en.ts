  ...
  projectDetail: {
    ...
    contentSection: {
      ...
      shopify: {
        ...
        hint: 'Connect a Shopify store with an Admin API access token. The token is stored encrypted and never shown again.',
        ...
        guide: {
          ...
          intro: 'Follow these steps in your Shopify admin to create a read-only Admin API token. It takes a few minutes.',
          steps: [
            'Open your Shopify store admin (yourstore.myshopify.com/admin).',
            'Open the apps and sales channels / developer area (Settings → Apps and sales channels → Develop apps).',
            'Create a new custom app (or select an existing one), and open its Admin API configuration.',
            'Under Admin API access scopes, enable ONLY these read permissions: read_products and read_content. Do not enable any write permissions.',
            'Install the app on your store.',
            'Open the API credentials tab and reveal/copy the Admin API access token (starts with shpat_).',
            'Copy your permanent store domain in the form store-name.myshopify.com.',
            'Back in Go Top, enter store-name.myshopify.com and paste the Admin API access token.',
            'Click Connect, then Test connection, then Sync now.',
          ],
          warningsTitle: 'Important',
          warnings: [
            'Use the *.myshopify.com domain, not only your public custom domain.',
            'Shopify shows the access token only once in some flows — copy and store it securely.',
            'Never send the token by email or WhatsApp.',
            'Go Top stores the token encrypted and never shows it again.',
            'Do not request write permissions — read_products and read_content are enough.',
            'Shopify’s menus differ by account and store type. If a menu path differs, look for the app’s Admin API configuration and installation screen rather than following one exact path.',
          ],
          docsUrl: 'https://help.shopify.com/en/manual/apps/app-types/custom-apps',
          docsLabel: 'Official Shopify guide: custom apps',
          close: 'Close',
        },
        ...
        errors: {
          ...
          missing_scopes: 'The token is missing required read scopes.',
