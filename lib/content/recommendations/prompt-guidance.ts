...
export function recommendationGuidance(langLabel: string, year: number, count: number): string {
  ...
  return [
    ...
    ``,
    `EXISTING PAGE OWNERSHIP: an existing product, category, collection or service page already OWNS its exact name as a keyword — that a dedicated blog article does not yet exist is NOT a reason to create one. NEVER set a new article's primaryKeyword to the exact name of an existing product/category/entity from the project context. Those names are LINK TARGETS and business context, not article keywords. A long-tail article about such an entity must state a DISTINCT question, problem, comparison, audience, occasion, maintenance/how-to or informational angle (e.g. "how to care for X over time", "which items pair with X"), and its primaryKeyword must be that longer, distinct query — never the bare entity name.`,
    ``,
    `OPPORTUNITY-FIRST: propose a real CONTENT OPPORTUNITY (a user need / search question / decision / problem / comparison / care-need / use-case / audience or occasion need / content-cluster gap) — NOT an existing entity re-expressed. For each idea state, at least implicitly, the user question it answers, its expected answer, and why it needs its own article. Prefer combining evidence: several entities that share a theme (a common care need, material, audience, use, decision or problem) become ONE broader guide that references them as targets — not one article per entity. Use each source for its strength: keyword-research signals for real demand, language and long-tail intent; indexed site pages/products/categories for ownership, coverage and link targets (never one article per scanned entity); project focus for priority and relevance; existing topics/articles for coverage and duplicate awareness. Do not claim search demand you have no evidence for.`,
    ``,
