...
export function buildOpportunityPrompt(clusters: EvidenceCluster[], ctx: ProjectContext, langLabel: string, year: number, count: number): string {
  return [
    ...
    ``,
    `From these clusters, propose up to ${count} DISTINCT CONTENT OPPORTUNITIES. Each must be a real user need / search question / decision / problem / comparison / care or use-case need — grounded in a cluster's demand_queries or project_focus. When a cluster has several commercial_entities sharing the theme, prefer ONE broader guide that references them as targets rather than one article per entity. NEVER use a commercial entity's bare name as primaryKeyword — entity names are link targets and context only. The primaryKeyword must describe the SAME need and intent as the title (a comparison title needs a comparison keyword, an informational title an informational keyword) and must not become a bare commercial/product phrase. Only mention demand ("high demand", "common searches") when the cluster's demand_queries include a real volume value (v > 0); otherwise use neutral business-relevance language and never fabricate search volume. Do not propose a topic a listed existing_coverage title already answers. Prefer clusters with missing_coverage and real demand; skip clusters with no defensible independent need.`,
