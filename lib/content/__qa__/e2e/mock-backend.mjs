  ...
  const server = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (path.startsWith('/rest/v1/')) {
        ...
        // Table-absent simulation (migration not applied): PostgREST answers 404 with
        // the undefined-table SQLSTATE 42P01. loadPendingIdeas/insertPendingIdeas map
        // this to null → the route's session-only path (persisted:false, 0 writes).
        if (Array.isArray(opts.absentTables) && opts.absentTables.includes(table)) {
        ...
        if (req.method === 'POST') {
          ...
          // Deterministic DB fingerprint-duplicate simulation: the first N payload rows for
          // content_topic_ideas are treated as ON CONFLICT DO NOTHING skips (a pre-existing
          // pending row), so `inserted` = attempted - N and `duplicate` = N — exactly the
          // real PostgREST behavior, without needing to predict fingerprints.
          const forcedDup = table === 'content_topic_ideas' ? (opts.contentIdeasDuplicates ?? 0) : 0
          const inserted = []
          let skippedForced = 0
          for (const row of payload) {
            if (skippedForced < forcedDup) { skippedForced++; continue }
...
export function startFakeGemini(cfg) {
  const calls = []
  // Stage D — MUTABLE scenario state the test flips between generations WITHOUT a
  // reboot: proMode ('ok' | 'provider_fail'), flashMode ('ok' | 'empty'),
  // proUnderYield (Pro returns a single valid topic → non-empty but < target).
  const state = cfg.state ?? { proMode: 'ok', flashMode: 'ok', proUnderYield: false }
  const server = createServer((req, res) => {
    let body = ''
    req.on('data', (c) => { body += c })
    req.on('end', () => {
      ...
      if ((req.url ?? '').includes(':generateContent')) {
        ...
        // STRICT structured-output validation (RC1): responseSchema is required.
        ...
        // CONSTRAINED-DISCOVERY prompt → anchored needs JSON.
        if (body.includes('OWNED ANCHORS')) {
          const am = body.match(/OWNED ANCHORS[^\[]*?(\[[\s\S]*?\])/)
          let anchors = []
          if (am) { try { anchors = JSON.parse(am[1].replace(/\\"/g, '"').replace(/\\n/g, '\n')) } catch { anchors = [] } }
          calls.push({ model, briefCount: -1, thinkingBudget })
          const needs = anchors.slice(0, 5).map((a, i) => ({ subject: i % 2 === 0 ? `יתרונות ${a} בשימוש יומיומי` : `טעויות נפוצות עם ${a}`, anchor: a, need: i % 2 === 0 ? 'explanation' : 'checklist', intent: 'informational' }))
          res.writeHead(200, { 'content-type': 'application/json' })
        ...
        const isFlash = !model.includes('pro')
        // Stage D scenarios — a Pro provider failure (non-fatal 400) drives the fallback.
        if (!isFlash && state.proMode === 'provider_fail') {
          calls.push({ model, briefCount: 0, thinkingBudget })
          res.writeHead(400, { 'content-type': 'application/json' })
          res.end(JSON.stringify({ error: { code: 400, message: 'Temporary bad request (non-fatal).', status: 'INVALID_ARGUMENT' } }))
          return
        }
        ...
        // Deterministic but STRUCTURALLY VARIED titles (question / mistakes /
        // myths / checklist / practical / focused …) — all subject-led or
        // suffix-framed so title↔keyword alignment holds, and at most ONE
        // mega-guide opening per batch (mirrors the real synthesis contract).
        const FRAMES = [
          (s) => `המדריך המלא: ${s}`,
          (s) => `${s}: שאלות ותשובות`,
          ...
          (s) => `${s} — המלצות מעשיות`,
          (s) => `${s}: צעד אחר צעד`,
          (s) => `${s} בבית: מה באמת עובד`,
          (s) => `${s} — נקודות מפתח לפני שמתחילים`,
        ]
        let topics = briefs.map((b, i) => ({
          briefId: b.id,
          title: FRAMES[i % FRAMES.length](b.subject),
          primaryKeyword: b.aligned_query ?? b.subject,
          secondaryKeywords: [],
          intent: 'informational',
        }))
        // Flash fallback returns nothing (Pro-0 + Flash-0 = no batch).
        if (isFlash && state.flashMode === 'empty') topics = []
        // Pro under-yield: a single valid topic (non-empty but below target).
        if (!isFlash && state.proUnderYield) topics = topics.slice(0, 1)
        res.writeHead(200, { 'content-type': 'application/json' })
  ...
  return new Promise((resolve) => server.listen(0, '127.0.0.1', () => resolve({ server, port: server.address().port, calls, state })))
}
...
export function e2eSeedTables() {
  ...
  // Stage D — a COLD-START project with NO evidence (no research / entities / tracking):
  // the deterministic pool is empty and discovery has no anchors, so a run yields NO
  // batch (genuine "no safe opportunities" empty state) and never runs Flash.
  const COLD = 'e2e00000-0000-4000-8000-00000000cold'
  return {
    ...
    projects: [
      { id: P, user_id: E2E_USER.id, is_active: true, business_name: 'הצמחייה - חנות הטבע', name: 'הצמחייה', target_domain: 'https://natural-shop.co.il', language: 'he', country: 'IL', created_at: '2026-01-01T00:00:00Z' },
      { id: COLD, user_id: E2E_USER.id, is_active: true, business_name: 'עסק חדש', name: 'קולד סטארט', target_domain: 'https://cold-start.example.co.il', language: 'he', country: 'IL', created_at: '2026-01-01T00:00:00Z' },
    ],
    tracking_targets: [{ project_id: P, keyword: 'תוספי תזונה טבעיים' }],
    keyword_research_cache: [{
      project_id: P, created_at: '2026-07-01T00:00:00Z', fetched_at: '2026-07-01T00:00:00Z', results_json: [
        ...
        // A real project carries many distinct keywords — so a second run (after
        // the first persisted its ideas) still has FRESH inventory rather than
        // re-suggesting owned/pending topics.
        { keyword: 'ברזל לנשים בהריון', avgMonthlySearches: 260 },
        { keyword: 'ויטמין B12 לצמחונים', avgMonthlySearches: 240 },
        { keyword: 'פרוביוטיקה לבריאות המעי', avgMonthlySearches: 190 },
        { keyword: 'קולגן לעור ולמפרקים', avgMonthlySearches: 300 },
        { keyword: 'אבץ לחיזוק מערכת החיסון', avgMonthlySearches: 150 },
        { keyword: 'ויטמין D במחסור', avgMonthlySearches: 280 },
        { keyword: 'סידן לחיזוק העצמות', avgMonthlySearches: 130 },
        { keyword: 'ספירולינה יתרונות בריאותיים', avgMonthlySearches: 110 },
      ],
    }],
    shopify_entities: [
...
export const E2E_COLDSTART_PROJECT_ID = 'e2e00000-0000-4000-8000-00000000cold'
