 ...
 * over fake providers, all eight production scenarios:
 *
...
let portBase = 3940
async function boot({ models = ['gemini-2.5-flash', 'gemini-2.5-pro'], geminiState, extraEnv = {}, absentTables = [] }) {
  const supa = await startMockSupabase(e2eSeedTables(), { verbose: false, absentTables })
...
async function main() {
  // ── BOOT A — Pro available, normal budget; scenarios 1–4 via mutable state ──
  {
    ...
    check('S3b. the user sees NO model/tier/fallback wording', !/Gemini|gemini-2\.5|Flash|\bPro\b|fallback|downgrade/i.test((await b.page.locator('body').innerText()).replace(/שפר עם Gemini Pro/g, '')))
    st.proMode = 'ok'
  ...
  // ── BOOT B — Pro UNAVAILABLE (models list has no Pro) → scenario 5 ──
  {
    ...
    check('S5b. user sees no fallback/model wording', !/Gemini|gemini-2\.5|Flash|\bPro\b|fallback|downgrade/i.test((await b.page.locator('body').innerText()).replace(/שפר עם Gemini Pro/g, '')))
