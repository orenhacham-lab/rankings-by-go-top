...
async function main() {
  ...
  // ── 3) Generate as PREMIUM via the real operator selector ──
  const selector = page.getByTestId('reco-quality-selector')
  check('operator quality selector is visible (flag-gated)', await selector.isVisible().catch(() => false))
  await selector.getByRole('button', { name: /Gemini Pro/ }).click().catch(() => {})
  ...
  check('reco-qa acceptance PASSED on the fixture project', qaData?.acceptance?.passed === true, JSON.stringify((qaData?.acceptance?.rules ?? []).filter((r) => !r.pass)))
