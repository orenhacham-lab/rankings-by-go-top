...
async function main() {
  ...
  check('compare PREFLIGHT returns a worst-case estimate + the enforced authorized limit (no spend)', cmp.pre?.preflight === true && typeof cmp.pre?.estimatedWorstCaseCostUsd === 'number' && typeof cmp.pre?.authorizedLimitUsd === 'number' && typeof cmp.pre?.withinAuthorizedLimit === 'boolean' && cmp.pre?.requiresConfirmation === cmp.pre?.withinAuthorizedLimit && cmp.pre?.persist === false, JSON.stringify(cmp.pre).slice(0, 240))
