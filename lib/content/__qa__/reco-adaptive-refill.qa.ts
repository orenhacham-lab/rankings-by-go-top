...
function main() {
  ...
  check('(F2 refill-identity) bounded refill = per-attempt round beyond the allowance (not global ordinal)', /const isBoundedRefillRound = allowRefill && round > legacyAttemptRounds/.test(gen) && /if \(isBoundedRefillRound\) \{[\s\S]{0,200}if \(!canRunBoundedRefill\(\)\) break/.test(gen))
  ...
  check('(F3 used-timing) thirdRefillUsed set ONLY after the controller-authorized call (after res.stopped guard)', gen.indexOf("if (res.stopped) { stop = 'budget_stopped'; break }") < gen.indexOf('if (isBoundedRefillRound) thirdRefillUsed = true') && !/thirdRefillUsed = true\n\s*\}\n\s*const deficit/.test(gen))
