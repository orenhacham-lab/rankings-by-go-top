...
function main() {
  ...
  // (1)(2)(3) total paid calls ≤ 3; no discovery → 3 synthesis rounds; discovery → 2.
  check('(1) paid-call cap is 3', /const PAID_CALL_CAP = 3/.test(gen))
  check('(2)(3) maxSynthesisRounds = cap − (discovery?1:0) → 3 without discovery, 2 with', /const maxSynthesisRounds = PAID_CALL_CAP - \(discovery\?\.ran \? 1 : 0\)/.test(gen))
  check('(1b) the loop is bounded by maxSynthesisRounds', /for \(let round = 1; round <= maxSynthesisRounds && !stop; round\+\+\)/.test(gen))
  check('(1c) callsRemaining reflects the 3-call cap', /callsRemaining: Math\.max\(0, PAID_CALL_CAP - totalPaidCalls\)/.test(gen))
  ...
  check('(refill-round) the 3rd paid call is the gated bounded refill', /const isBoundedRefillRound = \(round - 1\) \+ \(discovery\?\.ran \? 1 : 0\) >= 2/.test(gen) && /if \(isBoundedRefillRound\) \{[\s\S]{0,160}if \(!canRunBoundedRefill\(\)\) break/.test(gen))
  ...
  check('(5b) Flash fallback file (production-run) not modified for refill', !/thirdRefill|PAID_CALL_CAP|canRunBoundedRefill/.test(read('lib/content/recommendations/production-run.ts').replace(/thirdRefillEligible: false,\n\s*thirdRefillUsed: false,/, '')))
